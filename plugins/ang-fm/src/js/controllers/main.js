(function(angular, $) {
    ...
    angular.module('FileManagerApp').controller('FileManagerCtrl', [
        // '$scope', '$rootScope', '$window', '$translate', 'fileManagerConfig', 'item', 'fileNavigator', 'apiMiddleware', '$compile',
        // function($scope, $rootScope, $window, $translate, fileManagerConfig, Item, FileNavigator, ApiMiddleware, $compile) {
        ...
        $scope.smartClick = function(item) {
            ...
            if (item.isPdf()) {
                if ($scope.config.previewPdfsInModal) {
                    return $scope.openPdfPreview(item);
                } 
                return $scope.apiMiddleware.download(item, true);
            }
        ...
        $scope.openImagePreview = function() {
            ...
            var extension = item.model.extension;
            var source = $scope.apiMiddleware.getUrl(item);
            // fix podgląd miniatury plików "tif"
            if(extension == 'tif' || extension == 'tiff') {
                source = 'lib/img_thumbs.php?file='+item.model.filepath;
            }
        ...
        // lx.pandora.caps.pl/iws2/lib/pdf2jpg.php?pdf=cennik.pdf
        $scope.openPdfPreview = function() {
            var item = $scope.singleSelection();
            var pdf_file = $scope.apiMiddleware.getFilePath(item);
            $scope.apiMiddleware.apiHandler.inprocess = true;
            $scope.modal('imagepreview', null, true)
                .find('#imagepreview-target')
                .attr('src', 'lib/pdf2jpg.php?pdf=' + pdf_file)
                .unbind('load error')
                .on('load error', function() {
                    $scope.apiMiddleware.apiHandler.inprocess = false;
                    $scope.$apply();
        ...
        $scope.openInNewTab = function() {
            var item = $scope.singleSelection();
            var basePath = 'images/';
            var filePath = $scope.apiMiddleware.getFilePath(item).replace(/^\//, '');
            // $window.location.href = basePath + filePath;
            $window.open(basePath + filePath, '_blank');
            return $scope.apiMiddleware.getFilePath(item);
        };
        ...
        $scope.copyLink = function() {
            ...
            if (item) {
                var path = item.model.path.filter(Boolean);
                var filePath = ('/' + path.join('/') + '/' + item.model.name).replace(/\/\//, '/');
                
                // fix na podwójne slashe w chrome, jak zmenimy i w chrome jest dobrze to w innych przeglądarkach nie generuje poprawnie linków
                // var isChrome = !!window.chrome && !!window.chrome.webstore;                
                // if(isChrome) var fullPath = location.href.replace(/dokumenty\//gi, "documents") + filePath;
                // else var fullPath = location.href.replace(/dokumenty/gi, "documents") + filePath;
                var fullPath = location.href.replace(/#/gi, 'images') + filePath;
                // alert(fullPath);
                // window.stop();                  

                if(document.queryCommandSupported('copy')) {
                    var temp_clipboard = document.createElement('input');
                    document.body.appendChild(temp_clipboard);
                    temp_clipboard.setAttribute('id', 'temp_clipboard_id');
                    document.getElementById('temp_clipboard_id').value=fullPath;
                    temp_clipboard.select();
                    ...
                    $(temp_clipboard).css('display','none');
                    document.body.removeChild(temp_clipboard);
                }
                else {
