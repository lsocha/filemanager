...
class FileManagerApi
{
        ...
        switch ($request['action']) {
            case 'list':
                ...
                if (!is_array($list)) {
                    ...
                    $response_data = array('result' => $list);
            ...
            case 'getContent':
                ...
                if ($content !== false) {
                    $response = new Response();
                    $response_data = array('result' => $content);
                    $response->setData($response_data);
                } else {
                    $response = $this->simpleErrorResponse($t->file_not_found);
                }
    ...
    private function listAction($path)
    {
        ...
        $files = array_values(array_filter(
            ...
            function ($path) {
                ...
                return !($path === '.' || $path === '..' || $basename_path[0] === '.'); // wyrzucanie plików ukrytych
            }
        ));
        // $_this = $this;
        // function() use($_this) { ... }
        $_this = $this;
        $files = array_map(function ($file) use ($path, $_this) {
            $file = $_this->canonicalizePath(
                $_this->basePath . $path . DIRECTORY_SEPARATOR . $file
            );
            date_default_timezone_set('Europe/Warsaw');
            $date = new \DateTime('@' . filemtime($file));
            
            // sprawdzanie czy plik jest obrazem
            $image_types = array("jpeg", "jpg", "png", "gif", "bmp", "svg", "tiff", "tif");
            $file_extension = strtolower(pathinfo($file, PATHINFO_EXTENSION));
            if(in_array( $file_extension, $image_types)) $filetype = "image";
            else if($file_extension == "pdf") $filetype = "pdf";
            else $filetype = "file";

            // ścieżka do pliku
            $filePathFull = pathinfo($file, PATHINFO_DIRNAME)."/".pathinfo($file, PATHINFO_BASENAME);
            $filePath = substr($filePathFull, strpos($filePathFull, '/images/'));

            $return_data = array(
                'name' => pathinfo($file, PATHINFO_BASENAME),
                'rights' => $_this->parsePerms(fileperms($file)),
                'size' => filesize($file),
                'date' => $date->format('Y-m-d H:i:s'),
                // 'type' => is_dir($file) ? 'dir' : 'file',
                'type' => is_dir($file) ? 'dir' : $filetype,
                'filepath' => $filePath,
    ...
    private function removeAction($paths)
    {
        foreach ($paths as $path) {
            ...
            if (is_dir($path)) {
                ...
                $dirEmpty = $FilesystemIterator->valid();
    ...
    */
    ...
    {
        $name = preg_replace('~[^\\pL0-9_]+~u', '_', $name); // zamienia międzyinnymi spacje na podłogi
        $name = trim($name, "-");
        $name = iconv("utf-8", "us-ascii//TRANSLIT", $name);
        // $name = preg_replace('~[^-a-z0-9_]+~', '', $name); // zamiana wielkich liter na małe, zamienia je na puste znaki
        return $name;
    }
}
